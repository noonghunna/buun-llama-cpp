...
| VBR VMM future-reach ledger + MoE-cache budget integration + Stage-1 accounting test | prevents the cache's frozen free-now budget from consuming `p.budget - mapped` bytes that dynamic VBR has priced as future KV reach; map/unmap/destruction keep per-device outstanding bytes exact without touching the cross-process co-tenancy ledger | ours (fork issue #6; upstream candidate after the fixed-turbo non-regression and dynamic growth-to-full-context device gates) |
