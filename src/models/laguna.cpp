...
llama_model_laguna::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Unmasked nextn extraction needs the pre-final-norm state for every
        // token, so defer the output-row gather until after that capture.
    ...
    // Pre-final-norm residual stream: the DFlash training convention's
    // target-layer id equal to n_layer.
    cb(cur, "h_nextn", -1);
    res->t_h_nextn = cur;
